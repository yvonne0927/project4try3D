...
function initThreeOverlay() {
  ...
  three.renderer.domElement.style.zIndex = "10";
...
text(`model loaded: ${Object.keys(three.models).length}`, 20, 70);
  ...
  // ROI (center of cam)
  const roi = cam.get(
    cam.width / 2 - boxSize / 2,
    cam.height / 2 - boxSize / 2,
    boxSize,
    boxSize
  );
  ...
  fill(255);
  textSize(15);
  text(`edgePixels(ROI): ${edgePixels}`, 30, 50);
  text(`warmth (R-B): ${roiFeats.warmth.toFixed(1)}`, 30, 72);
  text(`brightness: ${roiFeats.brightness.toFixed(2)}`, 30, 94);
  text(`straightness: ${roiFeats.straightness.toFixed(2)} (lines: ${roiFeats.lineCount})`, 30, 116);
  text(`texture: ${roiFeats.texture.toFixed(2)}  smoothness: ${roiFeats.smoothness.toFixed(2)}`, 30, 138);
  text(`ROI organism: ${resultROI.organism}`, 30, 162);
  text(`ROI confidence: ${resultROI.confidence.toFixed(2)}`, 30, 184);

  text(`Tendril: ${resultROI.score.Tendril.toFixed(2)}`, 30, 210);
  text(`Glyph:   ${resultROI.score.GlyphLight.toFixed(2)}`, 30, 230);
  text(`Crystal: ${resultROI.score.CrystalShell.toFixed(2)}`, 30, 250);
  text(`Jelly:   ${resultROI.score.Jelly.toFixed(2)}`, 30, 270);
  text(`Spore:   ${resultROI.score.SporeCloud.toFixed(2)}`, 30, 290);
  ...
  if (three.ready && objects.length > 0) {
    ...
    if (three.current) {
      ...
      // 暂时固定位置
      three.current.position.set(0, -0.2, 0);
